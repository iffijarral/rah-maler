    ...
    if(method == "post" || method == "put"){
        ...
        for(let i = 0; i < attrs.length; i++){
            ...
            if( ! re.test(attrs[i].value) ){
                cl("mix-check failed")
                const pElementClass = 'error_message';

                // Check if the next sibling is an existing error message
                let nextSibling = attrs[i].nextSibling;
                while (nextSibling && nextSibling.nodeType !== 1) {
                    nextSibling = nextSibling.nextSibling;
                }

                if (nextSibling && nextSibling.classList && nextSibling.classList.contains(pElementClass)) {
                    // If the next sibling is the error message, update its text content
                    nextSibling.textContent = 'Please provide a valid value';
                } else {
                    // Create a new p element and insert it as the next sibling
                    const pElement = document.createElement('p');
                    pElement.textContent = 'Please provide a valid value';
                    pElement.classList.add(pElementClass);
                    attrs[i].parentNode.insertBefore(pElement, attrs[i].nextSibling);
                }
                ...
                if (nextSibling && nextSibling.classList && nextSibling.classList.contains(pElementClass)) {
                    // If the next sibling is the error message, remove it
                    nextSibling.remove();
                }
            }
        }  
